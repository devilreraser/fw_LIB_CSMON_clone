...
#ifndef HMLIB_MODBUS_DEBUG_RESPONSES
#define HMLIB_MODBUS_DEBUG_RESPONSES 0
#endif
#ifndef HMLIB_MODBUS_AND_HEXMON_UART_MODULE_DIFFERENT
#define HMLIB_MODBUS_AND_HEXMON_UART_MODULE_DIFFERENT 0
#endif

#ifndef HMDRV_MODBUS_SHARED_RX_TX_BUFFER
...
#define HMDRV_RECORDER_BUFFER1_SIZE 0x8000      /* Size In bytes (Freemaster supports only 16-bits size) */
#endif
...
#define HMDRV_RECORDER_BUFFER1_ADDR (HMDRV_ADDR)(EXT_MEM_RAM_VIRTUAL_ADDRESS + 0x78000)     /* at the end of 512 kB */
...
#define HMDRV_RECORDER_BUFFER0_ADDR (HMDRV_ADDR)(HMDRV_ADDR_SIZE)&au8RecBuffer0[0]
#define HMDRV_RECORDER_BUFFER1_ADDR (HMDRV_ADDR)(HMDRV_ADDR_SIZE)&au8RecBuffer0[0]
...
#define HMDRV_RECORDER_PARAM_BUFINT_COUNT 1   /* Used Module Internal Buffers Count */ 
#endif
...
#else
#define HMDRV_RECORDER_PARAM_BUFFER2_SIZE 0x8000      /* Size In bytes (Freemaster supports only 16-bits size) */
#endif
#endif

#define HMDRV_RECORDER_PARAM_BUFFER0_ADDR (HMDRV_ADDR)(HMDRV_ADDR_SIZE)&au8RecParamBuffer0[0]

#define HMDRV_RECORDER_PARAM_BUFFER1_ADDR (HMDRV_ADDR)(EXT_MEM_RAM_VIRTUAL_ADDRESS + 0x0000 )  /* in the beginning of extended memory */

#if HMDRV_USE_EXT_PMP
#define HMDRV_RECORDER_PARAM_BUFFER2_ADDR (HMDRV_ADDR)(EXT_MEM_PMP_VIRTUAL_ADDRESS)
#else
#define HMDRV_RECORDER_PARAM_BUFFER2_ADDR (HMDRV_ADDR)(EXT_MEM_RAM_VIRTUAL_ADDRESS + HMDRV_RECORDER_PARAM_BUFFER1_SIZE) /* after rec param1 */
#endif
...
#ifndef HMDRV_RECORDER_PARAM_BUFFER1_SIZE
#define HMDRV_RECORDER_PARAM_BUFFER1_SIZE_FREEMASTER    0xFFFF       /* Size In bytes (compatible with Freemaster supports only 16-bits size) 512kB(0x80000) leave 0xFFFF+1 for HMDRV_RECORDER_BUFFER1 */
#define HMDRV_RECORDER_PARAM_BUFFER1_SIZE 0xFFFF      /* Size In bytes (Freemaster supports only 16-bits size) */
#endif
...
#define HMDRV_RECORDER_PARAM_BUFFER0_ADDR (HMDRV_ADDR)(HMDRV_ADDR_SIZE)&au8RecParamBuffer0[0]
#define HMDRV_RECORDER_PARAM_BUFFER1_ADDR (HMDRV_ADDR)(HMDRV_ADDR_SIZE)&au8RecParamBuffer0[0]
#define HMDRV_RECORDER_PARAM_BUFFER2_ADDR (HMDRV_ADDR)(HMDRV_ADDR_SIZE)&au8RecParamBuffer0[0]
...
******************************************************************************/
#ifndef _WIN32
#define HMDRV_FILE_PNTR_MODIFIER __eds__
#define HMDRV_ADDR_PNTR_MODIFIER __eds__
#define HMDRV_BUFF_PNTR_MODIFIER __eds__
#define HMDRV_CNST_PNTR_MODIFIER __psv__
#else
...
typedef unsigned short  HMDRV_ADDR_TO_PNTR_SIZE; /* Size of Type Cast From ADDR to Internal RAM PNTR (without modifiers for Extended Data Space) */
#else
...
#define HMDRV_PTRFROMADDR(addr) addr
...
#define HMDRV_ValueFromBuffer16BigEndian(pDest, pSrc) \
    ( (*((HMDRV_ADDR_PNTR_MODIFIER HMDRV_U16*)(pDest)) = (HMDRV_U16)(*(HMDRV_U8*)(pSrc+1)) + ((HMDRV_U16)(*(HMDRV_U8*)(pSrc+0))<<8)), (((HMDRV_BPTR)(pSrc))+2) )
...
#define HMRDV_FixEdsAddress(x) (HMDRV_U32)x & 0xFF00FFFF     /* delete the 1 inside 0x0001XXXX when cast from eds address to u32 */   
...
#if 1
#define HMDRV_SCI_TE(eUartIndex)            UART_TransmitEnable(eUartIndex)
#define HMDRV_SCI_TD(eUartIndex)            UART_TransmitDisable(eUartIndex)
#else    
#define HMDRV_SCI_TE(eUartIndex)                /* not used  */
...
#define HMDRV_SETUP_TX_DELAY_FUNCTION(eUartIndex, pFunc)  TIMER_vSetupTimerMainDelayedFunctionSendData((UART_eModule_t)eUartIndex, pFunc)
...
/* Get received character */
#define HMDRV_SCI_GETCHAR(eUartIndex)    	 RC1REG
/* Rx Possible */
#define HMDRV_READY_RX(eUartIndex)        	((PIR1bits.RCIF == 0) ? false : true)
#endif

#if HMDRV_SCI_TX_ISR == 0
/* Tranmsit character */
#define HMDRV_SCI_PUTCHAR(eUartIndex, ch)	do {TX1REG = ch;} while(0)
/* Tx Possible */
#define HMDRV_READY_TX(eUartIndex)        	((PIR1bits.TXIF && TX1STAbits.TXEN) ? true : false)
/* Tx Complete/Empty */
#define HMDRV_EMPTY_TX(eUartIndex)        	((TX1STAbits.TRMT == 0) ? false : true)   
/* ISR Ena/Dis */    
//#define HMDRV_TX_INT_ENA(eUartIndex)       	
//#define HMDRV_TX_INT_DIS(eUartIndex)       	
#endif    

#define HMDRV_GET_FREE_RUN_TIMER() 			(HMDRV_TIMER)0
    
#else 
       
#if HMDRV_SCI_RX_ISR == 0   
/* Get received character */
#define HMDRV_SCI_GETCHAR(eUartIndex)     	U1RXREG  
/* Rx Possible */
#define HMDRV_READY_RX(eUartIndex)        	((U1STAbits.URXDA == 0) ? false : true)      
#endif

#if HMDRV_SCI_TX_ISR == 0
/* Tranmsit character */
#define HMDRV_SCI_PUTCHAR(eUartIndex, ch)  	do {U1TXREG = ch; } while(0)
/* Tx Possible */
#define HMDRV_READY_TX(eUartIndex)        	((U1STAbits.UTXBF == 0) ? true : false)
/* Tx Complete/Empty */
#define HMDRV_EMPTY_TX(eUartIndex)        	(( (U1STAbits.TRMT == 0)) ? false : true)   
/* ISR Ena/Dis */    
//#define HMDRV_TX_INT_ENA(eUartIndex)       	
//#define HMDRV_TX_INT_DIS(eUartIndex)       
#endif    
   
#define HMDRV_GET_FREE_RUN_TIMER() 			(HMDRV_TIMER)((HMDRV_TIMER)TMR8 + ((HMDRV_TIMER)TMR9HLD << 16))
...
#define HMDRV_DEBUG_PIN        ??? to do for pic     30      /* Amber LED (middle Led) On 1038 CC Board */
...
#define HMDRV_vInit()
#define HMDRV_vProcess()
#endif
#define HMDRV_vDebugSend()
...
#define HMDRV_GetStringLengthFromWord                   HMDRV_GetStringLength
 ...
 */
#define HMDRV_u32GetDeviceSYSCLKFreq() HMMODBUS_u32DeviceSYSCLKFreq /* Never Change Initialization Value */
