...
EDS_SPACE uint8_t u8SPIBuffer[SPIMEM_EXT_MEM_QUEUE_COUNT][SPIMEM_DATA_BUFFER_SIZE];
...
void vExtMemChipSelectEnable(uint16_t u16Memory)
{
    switch((eExtMemSelect_t)u16Memory)
    {
        ...
        case EXT_MEM_SEL_EEPROM:
            SPI_vSetFrequencyDivider(EXT_MEM_SEL_EEPROM, ePriDividerEeprom, eSecDividerEeprom);
            CS_EEPROM_SetLow(); /* SPI_CHIP_SELECT_ENABLE */
            break;
        #endif
        #if USE_SPI_RAM
        case EXT_MEM_SEL_RAM:
            SPI_vSetFrequencyDivider(EXT_MEM_SEL_RAM, ePriDividerRam, eSecDividerRam); 
            CS_RAM_SetLow(); /* SPI_CHIP_SELECT_ENABLE */
            break;
        #endif
        #if USE_SPI_FLIR
        case EXT_MEM_SEL_FLIR:
            SPI_vSetFrequencyDivider(EXT_MEM_SEL_FLIR, ePriDividerFlir, eSecDividerFlir); 
            CS_FLIR_SetLow(); /* SPI_CHIP_SELECT_ENABLE */
            break;
        #endif
        #if USE_SPI_FPGA
        case EXT_MEM_SEL_FPGA:
            SPI_vSetFrequencyDivider(EXT_MEM_SEL_FPGA, ePriDividerFpga, eSecDividerFpga); 
...
void SPIMEM_vInit(uint32_t u32Fcy)
{
    ...
    SPI_vChipSelectFunctionsInit(eExtMemSPIModule, vExtMemChipSelectEnable, vExtMemChipSelectDisable );
    
#if SPIMEM_EXT_MEM_USE_SPIMEM_QUEUE_OLD                              
    SPI_vRxCompleteFunctionsInit(eExtMemSPIModule, vRxCompleteProcess );
#endif

    SPI_vCalcSPIPrescaler(u32SPIRamHz,u32Fcy, &u16SPIPrescalerRam);
    SPI_vCalcSPIPrescaler(u32SPIEepromHz,u32Fcy, &u16SPIPrescalerEeprom);
#if USE_SPI_FLIR
    SPI_vCalcSPIPrescaler(u32SPIFlirHz,u32Fcy, &u16SPIPrescalerFlir);
#endif
    
    SPI_vInitSPIPrescalers(u16SPIPrescalerRam,&ePriDividerRam, &eSecDividerRam);
    SPI_vInitSPIPrescalers(u16SPIPrescalerEeprom,&ePriDividerEeprom, &eSecDividerEeprom);
#if USE_SPI_FLIR
    SPI_vInitSPIPrescalers(u16SPIPrescalerFlir,&ePriDividerFlir, &eSecDividerFlir);
#endif
 ...
 **************************************************************************** */
void SPIMEM_vSPICommandQueue(eExtMemSelect_t eMemoryIndex, eExtMemCommand_t command, uint32_t address, uint16_t size, HMDRV_ADDR_PNTR_MODIFIER sSPIDMATransferBuffer_t* buffer)
{  
...
HMDRV_ADDR SPIMEM_pExtMemRamToIntRam(HMDRV_ADDR nSrcAddr, HMDRV_SIZE8 nSizeReq, HMDRV_ADDR_PNTR_MODIFIER HMDRV_SIZE8* pnSizeRead, HMDRV_BUFF_PNTR_MODIFIER HMDRV_U16* pu16GlobalQueueID)
{
    HMDRV_ADDR_PNTR_MODIFIER HMDRV_U8* ps = NULL;
    
    uint16_t u16BufferIndex;
    uint16_t u16Count;
    HMDRV_ADDR_PNTR_MODIFIER sSPIDMATransferBuffer_t* buffer;
    eExtMemSelect_t eMemoryIndex;
    ...
    *pu16GlobalQueueID = u16BufferIndex;
    ...
    {
        buffer = (HMDRV_ADDR_PNTR_MODIFIER sSPIDMATransferBuffer_t*)u8SPIBuffer[u16BufferIndex];
        ...
        *pnSizeRead = nSizeReq;
        do
        {
            SPI_ReadWriteBuffer ( (eExtMemSPIModule | (eMemoryIndex << 4)), (HMDRV_ADDR_PNTR_MODIFIER uint8_t*)buffer, u16Count, &eStatus, NULL); 
        ...
        {
            ps = (HMDRV_ADDR_PNTR_MODIFIER HMDRV_U8*)(&(buffer->Data));
        }
        else
        {
...
HMDRV_ADDR SPIMEM_pExtMemRamFromIntRam(HMDRV_ADDR nDestAddr, HMDRV_SIZE8 nSize, HMDRV_BUFF_PNTR_MODIFIER HMDRV_U16* pu16GlobalQueueID) 
{
    HMDRV_ADDR_PNTR_MODIFIER HMDRV_U8* pd = NULL;
