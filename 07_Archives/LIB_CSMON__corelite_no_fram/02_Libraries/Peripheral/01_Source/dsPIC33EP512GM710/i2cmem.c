...
#define I2CMEM_MCP79410_7BIT    0x6F            /* 0xDE / 2         -> 111 ( MCP79410-I/MS ) */  
#define I2CMEM_DS3231MZ_7BIT    0x68            /* 0xD0 / 2         -> 104 ( DS3231MZ ) */  

#if 1
#define RTCC_SRAM_ADDR_7BIT     I2CMEM_DS3231MZ_7BIT
#else
#define RTCC_SRAM_ADDR_7BIT     I2CMEM_MCP79410_7BIT
#endif
...
typedef struct 
{
    ...
    uint16_t u16I2CCmdAddress;
    HMDRV_ADDR_PNTR_MODIFIER uint8_t* pI2CCmdBuffer;
    uint16_t u16I2CCmdSize;
    uint8_t u8I2CAddrSize;
    eI2CCmd_t eI2CCmdCommand;
    volatile eI2CCmdState_t eI2CCmdState;
    uint16_t rtrycntr;
    uint16_t cntr;
    uint8_t u8UseAckPoolingWaitWriteComplete;       /* Use Ack Pool To Wait Write Complete - Default Off */
...
sI2CNonMCCData_t sI2CNonMCCData[I2C_COUNT] = 
{
    {
...
sRTCC_t RTCC;
HMDRV_ADDR_PNTR_MODIFIER uint8_t SRAM_RTCC[64];
HMDRV_ADDR_PNTR_MODIFIER uint8_t EEPROM_RTCC[256];
HMDRV_ADDR_PNTR_MODIFIER uint8_t PICAUX[256];
...
HMDRV_ADDR_PNTR_MODIFIER uint8_t u8I2CBufferRD[I2C_COUNT][256];
HMDRV_ADDR_PNTR_MODIFIER uint8_t u8I2CBufferWR[I2C_COUNT][256];
HMDRV_ADDR_PNTR_MODIFIER uint8_t* pI2CBufferRD[I2C_COUNT] = {u8I2CBufferRD[I2C_1],u8I2CBufferRD[I2C_2]};
HMDRV_ADDR_PNTR_MODIFIER uint8_t* pI2CBufferWR[I2C_COUNT] = {u8I2CBufferWR[I2C_1],u8I2CBufferWR[I2C_2]};
...
void I2CMEM_vCopyToExtMem(HMDRV_ADDR nDestAddr, HMDRV_BPTR pSrcBuff, HMDRV_SIZE8 nSize)
{
    ...
    HMDRV_ADDR_PNTR_MODIFIER HMDRV_U8* pb;
...
void I2CMEM_vCopyToExtMemWithMask(HMDRV_ADDR nDestAddr, HMDRV_BPTR pSrcBuff, HMDRV_SIZE8 nSize)
{
    ...
    HMDRV_ADDR_PNTR_MODIFIER HMDRV_U8* pbRD;
    HMDRV_ADDR_PNTR_MODIFIER HMDRV_U8* pbWR;
    HMDRV_SIZE8 size = nSize;
 ...
 **************************************************************************** */
//HMDRV_ADDR_PNTR_MODIFIER HMDRV_BPTR I2CMEM_pExtMemRamToIntRam(HMDRV_ADDR nSrcAddr, HMDRV_SIZE8 nSize)
...
void I2CMEM_vProcessCmd(void)
{
    ...
    switch (eI2CCmd)
    {
        ...
        case I2C_CMD_RTCC_WR:
            I2CMEM_vWrite(I2C_CTRL_BYTE_RTCC_SRAM_WR, u16I2CAddress, (HMDRV_ADDR_PNTR_MODIFIER uint8_t*)&RTCC + u16I2CAddress, u16I2CSizeWR, 1, u8I2CModule);
            break;
            
        case I2C_CMD_RTCC_RD:
            I2CMEM_vRead(I2C_CTRL_BYTE_RTCC_SRAM_RD, u16I2CAddress, (HMDRV_ADDR_PNTR_MODIFIER uint8_t*)&RTCC + u16I2CAddress, u16I2CSizeRD, 1, u8I2CModule);
            break;
            
        case I2C_CMD_SRAM_WR:
            I2CMEM_vWrite(I2C_CTRL_BYTE_RTCC_SRAM_WR, u16I2CAddress + EXT_MEM_RTCC_SRAM_ADDRESS_OFFSET, (HMDRV_ADDR_PNTR_MODIFIER uint8_t*)&SRAM_RTCC + u16I2CAddress, u16I2CSizeWR, 1, u8I2CModule);
            break;
            
        case I2C_CMD_SRAM_RD:
            I2CMEM_vRead(I2C_CTRL_BYTE_RTCC_SRAM_RD, u16I2CAddress + EXT_MEM_RTCC_SRAM_ADDRESS_OFFSET, (HMDRV_ADDR_PNTR_MODIFIER uint8_t*)&SRAM_RTCC + u16I2CAddress, u16I2CSizeRD, 1, u8I2CModule);
            break;
            
        case I2C_CMD_EEPROM_WR:
            I2CMEM_vWrite(I2C_CTRL_BYTE_EEPROM_WR, u16I2CAddress, (HMDRV_ADDR_PNTR_MODIFIER uint8_t*)&EEPROM_RTCC + u16I2CAddress, u16I2CSizeWR, 1, u8I2CModule);
            break;
            
        case I2C_CMD_EEPROM_RD:
            I2CMEM_vRead(I2C_CTRL_BYTE_EEPROM_RD, u16I2CAddress, (HMDRV_ADDR_PNTR_MODIFIER uint8_t*)&EEPROM_RTCC + u16I2CAddress, u16I2CSizeRD, 1, u8I2CModule);
            break;
            
        case I2C_CMD_PICAUX_WR:
            I2CMEM_vWrite(I2C_CTRL_BYTE_PICAUX_WR, u16I2CAddress, (HMDRV_ADDR_PNTR_MODIFIER uint8_t*)&PICAUX + u16I2CAddress, u16I2CSizeWR, 1, u8I2CModule);
            break;
            
        case I2C_CMD_PICAUX_RD:
            I2CMEM_vRead(I2C_CTRL_BYTE_PICAUX_RD, u16I2CAddress, (HMDRV_ADDR_PNTR_MODIFIER uint8_t*)&PICAUX + u16I2CAddress, u16I2CSizeRD, 1, u8I2CModule);
            break;
...
{
    bool result = false;
    if (sI2CNonMCCData[eModule].eI2CCmdState == I2C_CMD_STA_IDLE)
    {
        result = true;
    }
    return result;
...
{
    sI2CNonMCCData_t* psI2CData = (sI2CNonMCCData_t*)&sI2CNonMCCData[u8I2CModule];
    
    if (psI2CData->eI2CCmdState == I2C_CMD_STA_IDLE)
    {
        eI2CCmd = I2C_CMD_BUSY;  /* consider to be reworked in new version */
        
        psI2CData->eI2CCmdCtrlByte = u16AddressI2C & (~(0x0001));   /* write command */
        psI2CData->u16I2CCmdAddress = u16Address;
        psI2CData->pI2CCmdBuffer = pdData;
        psI2CData->u16I2CCmdSize = u16Size;
        psI2CData->u8I2CAddrSize = u8AddressSize;
        psI2CData->eI2CCmdCommand = I2C_CMD_WR;
        
        psI2CData->rtrycntr = 0;
        psI2CData->cntr = 0;
        ...
        if (u8I2CModule == 0)
        {
            #if USE_I2C_ISR == 1
            // Start Condition
            psI2CData->eI2CCmdState = I2C_CMD_STA_START_WAIT;
            I2C1CONbits.SEN = 1;
            #else
            psI2CData->eI2CCmdState = I2C_CMD_STA_START;
            #endif
        }
        else
        {
...
void I2CMEM_vWrite(uint16_t u16AddressI2C, uint16_t u16Address, HMDRV_ADDR_PNTR_MODIFIER uint8_t* pdData, uint16_t u16Size, uint8_t u8AddressSize, uint8_t u8I2CModule)
{
    ...
    if (u8I2CModule == 0)
    {
        ...
        I2C1_MasterWrite(pu8I2CBufferMCCUse, index, 
                (u16AddressI2C >> 1), (I2C1_MESSAGE_STATUS*)pu8StatusTransaction); 
    }
    else
    {
        while (I2C2_MasterQueueIsFull()){;}
        I2C2_MasterWrite(pu8I2CBufferMCCUse, index, 
                (u16AddressI2C >> 1), (I2C2_MESSAGE_STATUS*)pu8StatusTransaction); 
    }
...
void I2CMEM_vRead(uint16_t u16AddressI2C, uint16_t u16Address, HMDRV_ADDR_PNTR_MODIFIER uint8_t* pdData, uint16_t u16Size, uint8_t u8AddressSize, uint8_t u8I2CModule)
{
    ...
    {
        eI2CCmd = I2C_CMD_BUSY;  /* consider to be reworked in new version */
        psI2CData->eI2CCmdCtrlByte = u16AddressI2C | (0x0001);
        psI2CData->u16I2CCmdAddress = u16Address;
        psI2CData->pI2CCmdBuffer = pdData;
        psI2CData->u16I2CCmdSize = u16Size;
        psI2CData->u8I2CAddrSize = u8AddressSize;
        psI2CData->eI2CCmdCommand = I2C_CMD_RD;
...
void vCopyFromI2CBuffer(uint16_t u16Size, HMDRV_ADDR_PNTR_MODIFIER uint8_t* pdData)
{
    uint16_t u16Index;
...
void I2CMEM_vRead(uint16_t u16AddressI2C, uint16_t u16Address, HMDRV_ADDR_PNTR_MODIFIER uint8_t* pdData, uint16_t u16Size, uint8_t u8AddressSize, uint8_t u8I2CModule)
{
    ...
    if (u8I2CModule == 0)
    {
        ...
        while (I2C1_MasterQueueIsFull());
        I2C1_MasterWrite(pu8I2CBufferMCCUse, index, (u16AddressI2C >> 1), (I2C1_MESSAGE_STATUS*)pu8StatusTransaction); 
        do
        {
            Nop();
        }
        while (I2C1_MasterQueueIsFull());
        I2C1_MasterRead(pu8I2CBufferMCCUse, size, (u16AddressI2C >> 1), (I2C1_MESSAGE_STATUS*)pu8StatusTransaction); /* Select line above if the whole function returns the status instead of this line and the line below */ 
    }
    else
    {
        do
        ...
        while (I2C2_MasterQueueIsFull());
        I2C2_MasterWrite(pu8I2CBufferMCCUse, index, (u16AddressI2C >> 1), (I2C2_MESSAGE_STATUS*)pu8StatusTransaction); 
        do
        {
            Nop();
        }
        while (I2C2_MasterQueueIsFull());
        I2C2_MasterRead(pu8I2CBufferMCCUse, size, (u16AddressI2C >> 1), (I2C2_MESSAGE_STATUS*)pu8StatusTransaction); /* Select line above if the whole function returns the status instead of this line and the line below */
    }
...
void I2CMEM_vProcessRWISR2(void)
{    
    sI2CNonMCCData_t* psI2CData = (sI2CNonMCCData_t*)&sI2CNonMCCData[I2C_2];
    
    switch (psI2CData->eI2CCmdState)
    {
        /*==================================*/
...
//            break;
        case I2C_CMD_STA_START_WAIT:
            ...
            if( psI2CData->jDone == 1 )
            {
                psI2CData->jDone = 0;
                //    I2C2TRN=(0x00A0)|(((i2cMem->oData->csel)&0x7)<<1);
                I2C2TRN = psI2CData->eI2CCmdCtrlByte & (~(0x0001));    /* Always Write command for Address */
                psI2CData->eI2CCmdState = I2C_CMD_STA_SEND_CTRL;
            }
            break;
       case I2C_CMD_STA_SEND_CTRL:
            ...
            if( psI2CData->jDone == 1 )
            {
                ...
                {                   // Ack Not received, Retry
                    if( psI2CData->rtrycntr < I2C_MAX_RETRY )
                    {
                        //eI2CCmdState = I2C_CMD_STA_RETRY;
                        I2C2CONbits.PEN = 1;
                        psI2CData->eI2CCmdState = I2C_CMD_STA_RETRY_END;
                        psI2CData->rtrycntr++;
                    }
                    else
                    {
                        ...
                        I2C2CONbits.PEN = 1;
                        psI2CData->eI2CCmdState = I2C_CMD_STA_ERROR_END;
                    }
                }
                else
                {
                    psI2CData->rtrycntr = 0;
                    if (psI2CData->u8I2CAddrSize == 2)
                    {
                    //#if I2C_ADDRWIDTH == TWO_BYTE
                        I2C2TRN = (psI2CData->u16I2CCmdAddress)  >> 8;
                        psI2CData->eI2CCmdState = I2C_CMD_STA_SEND_ADDRESS_HI;
                    //#endif
                    }
                    else /* if (u8I2CAddrSize == 1) */
                    {
                    //#if I2C_ADDRWIDTH == ONE_BYTE
                        I2C2TRN = (psI2CData->u16I2CCmdAddress);
                        psI2CData->eI2CCmdState = I2C_CMD_STA_SEND_ADDRESS_LO;
                    //#endif
                    }
                }
            }
         ...
         case I2C_CMD_STA_SEND_ADDRESS_HI:
            ...
            if( psI2CData->jDone == 1 )
            {
                ...
                else
                {
                    ...
                    {
                    //#if I2C_ADDRWIDTH == TWO_BYTE
                        I2C2TRN = ( (psI2CData->u16I2CCmdAddress) & 0x00FF );
                    //#endif
                    }
                    psI2CData->eI2CCmdState = I2C_CMD_STA_SEND_ADDRESS_LO;
                }
        ...
        case I2C_CMD_STA_SEND_ADDRESS_LO:
            ...
            if( psI2CData->jDone == 1 )
            {
                ...
                {
                    if( psI2CData->eI2CCmdCommand == I2C_CMD_WR )
                    {
                        //eI2CCmdState = I2C_CMD_STA_SEND_DATA;
                        
                        if (psI2CData->u8I2CAddrSize == 2) /* 16-bit data MSB First */
                        {
                            if ((psI2CData->cntr & 1) == 0)
                            {
                                I2C2TRN = *( psI2CData->pI2CCmdBuffer + psI2CData->cntr + 1 );
                            }
                            else
                            {
                                I2C2TRN = *( psI2CData->pI2CCmdBuffer + psI2CData->cntr - 1 );
                        ...
                        {
                            I2C2TRN = *( psI2CData->pI2CCmdBuffer + psI2CData->cntr );
                        }
                        
                        psI2CData->eI2CCmdState = I2C_CMD_STA_SEND_DATA_ACK;
                        psI2CData->cntr = psI2CData->cntr + 1;
                    }

                    if( psI2CData->eI2CCmdCommand == I2C_CMD_RD )
                    {
                        //eI2CCmdState = I2C_CMD_STA_READ_DATA_START;
                        // Repeat Start
                        I2C2CONbits.RSEN = 1;
                        psI2CData->eI2CCmdState = I2C_CMD_STA_READ_DATA_CONTROL;
                    }
                }
            }
...
//            break;
        ...
        case I2C_CMD_STA_SEND_DATA_ACK:
            ...
            if( psI2CData->jDone == 1 )
            {
                ...
                {
                    if( psI2CData->cntr == psI2CData->u16I2CCmdSize )
                    {  
                        if (psI2CData->u8UseAckPoolingWaitWriteComplete)
                        {
                            ...
                            I2C2CONbits.PEN = 1;
                            psI2CData->eI2CCmdState = I2C_CMD_STA_STOP_END;
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        psI2CData->eI2CCmdState = I2C_CMD_STA_SEND_DATA_ACK;//not needed state not changed
                        psI2CData->cntr = psI2CData->cntr + 1;
                    }
                }
            }
...
//            break;
        ...
        case I2C_CMD_STA_READ_DATA_CONTROL:
            ...
            {
                psI2CData->jDone = 0;
                psI2CData->eI2CCmdState = I2C_CMD_STA_READ_DATA_CONTROL_ACK;
                I2C2TRN = psI2CData->eI2CCmdCtrlByte | (0x0001);   /* Always read data request here */
            }
            break;
        ...
        case I2C_CMD_STA_READ_DATA_CONTROL_ACK:
            ...
            if( psI2CData->jDone == 1 )
            {
                ...
                else
                {
                    ...
                    I2C2CONbits.RCEN = 1;
                    psI2CData->eI2CCmdState = I2C_CMD_STA_READ_DATA_RECEIVE_ACK;                    
                }
            }
            break;
...
//            break;
        ...
        case I2C_CMD_STA_READ_DATA_RECEIVE_ACK:
            ...
            {
                psI2CData->jDone = 0;
                psI2CData->eI2CCmdState = I2C_CMD_STA_READ_DATA_RECEIVE_END;

                if (psI2CData->u8I2CAddrSize == 2) /* 16-bit data MSB First */
                {
                    if ((psI2CData->cntr & 1) == 0)
                    {
                        *( psI2CData->pI2CCmdBuffer + psI2CData->cntr + 1) = I2C2RCV;
                    }
                    else
                    {
                        *( psI2CData->pI2CCmdBuffer + psI2CData->cntr - 1) = I2C2RCV;
                ...
                {
                    *( psI2CData->pI2CCmdBuffer + psI2CData->cntr ) = I2C2RCV;
                }
...
//            break;
        ...
        case I2C_CMD_STA_STOP_END:
            if( psI2CData->jDone == 1 )
            {
                psI2CData->jDone = 0;
                eI2CCmd = I2C_CMD_IDLE;
                psI2CData->eI2CCmdState = I2C_CMD_STA_IDLE;
                psI2CData->cntr = 0;
                psI2CData->eI2CCmdCommand = I2C_CMD_IDLE;
            }
...
//            break;
        ...
        case I2C_CMD_STA_ERROR_END:
            if( psI2CData->jDone == 1 )
            {
                ...
                psI2CData->rtrycntr = 0;
                psI2CData->cntr = 0;
                psI2CData->eI2CCmdCommand = 0xFFFF;
            }
...
void I2CMEM_vProcessRWISR(void)
{ 
    sI2CNonMCCData_t* psI2CData = (sI2CNonMCCData_t*)&sI2CNonMCCData[I2C_1];
...
//            break;
        case I2C_CMD_STA_START_WAIT:
            ...
            if( psI2CData->jDone == 1 )
            {
                ...
                I2C1TRN = psI2CData->eI2CCmdCtrlByte & (~(0x0001));    /* Always Write command for Address */
                psI2CData->eI2CCmdState = I2C_CMD_STA_SEND_CTRL;
            }
            break;
       case I2C_CMD_STA_SEND_CTRL:
            ...
            if( psI2CData->jDone == 1 )
            {
                ...
                else
                {
                    ...
                    {
                    //#if I2C_ADDRWIDTH == TWO_BYTE
                        I2C1TRN = (psI2CData->u16I2CCmdAddress)  >> 8;
                        psI2CData->eI2CCmdState = I2C_CMD_STA_SEND_ADDRESS_HI;
                    //#endif
                    }
                    else /* if (u8I2CAddrSize == 1) */
                    {
                    //#if I2C_ADDRWIDTH == ONE_BYTE
                        I2C1TRN = (psI2CData->u16I2CCmdAddress);
                        psI2CData->eI2CCmdState = I2C_CMD_STA_SEND_ADDRESS_LO;
                    //#endif
                    }
                }
         ...
         case I2C_CMD_STA_SEND_ADDRESS_HI:
            ...
            if( psI2CData->jDone == 1 )
            {
                ...
                else
                {
                    ...
                    {
                    //#if I2C_ADDRWIDTH == TWO_BYTE
                        I2C1TRN = ( (psI2CData->u16I2CCmdAddress) & 0x00FF );
                    //#endif
                    }
                    psI2CData->eI2CCmdState = I2C_CMD_STA_SEND_ADDRESS_LO;
                }
...
//            break;
        ...
        case I2C_CMD_STA_READ_DATA_CONTROL:
            ...
            if( psI2CData->jDone == 1 )
            {
                psI2CData->jDone = 0;
                psI2CData->eI2CCmdState = I2C_CMD_STA_READ_DATA_CONTROL_ACK;
                I2C1TRN = psI2CData->eI2CCmdCtrlByte | (0x0001);   /* Always read data request here */
            }
            break;
